...
**Chloe 說「寫文章 / 挑主題 / 今天要幾篇 / 有什麼可寫」時，第一件事是查現有題材庫——不是憑空發想。**

1. **讀 `./memory/topics-master.md`** — 看已產出、待確認、已確認的主題清單與 status
2. **必要時查 Google Sheet**（ID: `1RxSb3bArkcqVQmQyM6ppfRm9GAXCjdHxHLCgIWxro8U`）看最新 status 欄位
3. **篩出 `ideator 推薦` / 待 Chloe 確認 的主題**先給 Chloe 選
4. **只有在現有清單全部不合用時**，才考慮產新主題；且要先說明「為什麼舊的不合用」

**違反此步驟 = 重大流程錯誤**。ideator 每天 13:00 自動跑，Sheet 裡永遠有待選主題，不需要憑空想。
...
### 建議也讀（如果你有建立）
10. 自己的業務脈絡檔（例：`./memory/business-overview.md`）— 你公司的三支柱、服務清單、當季重點
11. 自己的選題規則與禁忌（例：`./memory/topic-selection-rules.md`）
12. Analytics Agent 提供的數據摘要（如果 Analytics 有跑）
...
**管理課程：** 從智谷實際在開的課程和服務反推選題。有真實案例的優先（內部案例庫 裡的[3C 零售業者]OKR、[科技製造業]績效、[精品業] 服務稽核等）。GSC 已證明「主管」「領導風格」「MBO」「OKR」等詞有搜尋量。
...
5. **🚨 存完 brief 後立刻跑 pipeline_sync 把 Sheet status 同步**：
   ```bash
   cd automation && python3 -m ideator.pipeline_sync
   ```
...
- **手動選題（不是從 Sheet 來）** → 填 `手動選題` 四個字
- **任何情況都不能留空**——留空就是壞掉的 brief
